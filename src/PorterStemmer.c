...
/**
 * @brief Ejecuta el algoritmo de extracción de raíces de Porter sobre str
 * @param str cadena de entrada sobre la que ejecutar el algoritmo
 * @return Cadena resultado
 * @post Reserva memoria para la cadena devuelta.
 */
char *stemstring(const char *str) {
    char *toRet;
    int ch, count=0, i;
    char *tmp;
    //Reservamos memoria.
    ...
    while(TRUE) {
        ...
        if (LETTER(ch)) {
         ...
         /* the previous line calls the stemmer and uses its result to
            zero-terminate the string in s */
            if (s[0]!='\0') sprintf(toRet,"%s %s",toRet, s);

      }
    }
    free(s); //Liberamos memoria temporal

    tmp=(char *)malloc((strlen(toRet)+1)*sizeof(char));
    strcpy(tmp, toRet);
    free(toRet);
    toRet=tmp;
    if (toRet[0]==' ') { return toRet+1; } //Hago esto porque toma un espacio inicial.
    return toRet;
}
...
{
    char cad[2000]={"might have a wad of cash already laid aside others might be thinking of jumping ship to one of the iPhones numerous competitors or just getting some more months of utility out of their current iPhone"};
    char *ret;
   printf("cad: %s\n",cad);
   ret=stemstring(cad);
   printf("Sale: %s\n",ret);
